...
3. **Categorize**: group stored *spend* (positive-amount, i.e. outflow, transactions - refunds/deposits are excluded) by category, defaulting anything with no Plaid category to `"unknown"` - no Plaid call, just sqlite.
...
`GET /v1/transactions/categories?user_id=124`

    response: {
        "categories": [
            {"category": "FOOD_AND_DRINK", "amount": 145.32, "count": 12, "percentage": 23.4},
            {"category": "unknown", "amount": 20.00, "count": 2, "percentage": 3.2}
        ],
        "total_spending": 621.15
    }

### Notes

- `category` is Plaid's `personal_finance_category.primary` field.
- Not implemented (yet): full `/transactions/sync` cursor-based reconciliation (proper pending→posted transitions), duplicate/reversed-transaction detection, and an endpoint to manually edit a transaction's category - all reasonable next steps once the basic pull/view loop is in daily use.
